...
const GoTo: React.FC<Props> = ({ location, setLocation }) => {
  const [lat, setLat] = useState(location.latitude ? `${location.latitude}` : '34.086')
  const [lng, setLng] = useState(location.longitude ? `${location.longitude}` : '-118.192')
  const [zoom, setZoom] = useState(location.zoom ? `${location.zoom}` : '18')
